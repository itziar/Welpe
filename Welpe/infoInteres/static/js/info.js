...
function manage_likeInfo(id, url) {
    if (url) {
        url = '/' + url + '/like'
    } else {
        url = 'like'
    ...
    $.ajax({
        ...
        type: 'post',
        sync: true,
        data: $('#likeInfo_form_' + id).serialize(),
        dataType: 'json',
        success: function (data) {
            if (id.indexOf("info") == 0) {
                ...
                change_text_to_show(id, data);
                change_text_to_show('likeInfo' + id, data);
            }
        },
        error: function (data) {
...
function change_text_to_show(id, data) {
    ...
    if (data["status"] === "unliked") {
        $("#info_like_" + id).html(data["number_of_likes"]);
        // TODO: use i18n engine
        $("#submit_likeInfo_" + id).removeClass("corazon");
        $("#submit_likeInfo_" + id).addClass("corazon_gray");
        $("#card_" + id).toggleClass("panel-success");
    } else {
        $("#info_like_" + id).html(data["number_of_likes"]);
        // TODO: use i18n engine
        $("#submit_likeInfo_" + id).removeClass("corazon_gray");
        $("#submit_likeInfo_" + id).addClass("corazon");
        $("#card_" + id).toggleClass("panel-success");
    }
}
